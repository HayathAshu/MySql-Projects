...
select 
	date_format(start_date, '%Y-%m-01') as Month_start, count(*) as trial_starts from subscriptions
    where plan_id = 0
...
select p.plan_name, count(*) as plan_start_count from subscriptions as s
join
...
select 
    ...
    round(
		count(distinct case when plan_id = 4 then customer_id end) * 100.0 /
        count(distinct customer_id),
        1
        ) as churn_percentage from subscriptions;
        
# 5. How many customers have churned straight after their initial free trial - what percentage is this rounded to the nearest whole number?
select 
	count(*) as churn_after_trial,
    round(count(*) * 100.0 / (select count(distinct customer_id) from subscriptions)) as pct_churn_after_trail
from(
...
SELECT 
    ...
    ROUND(COUNT(*) * 100.0 / (SELECT COUNT(DISTINCT customer_id) FROM subscriptions), 1) AS pct_customers
FROM subscriptions s
JOIN plans p 
  ON s.plan_id = p.plan_id
WHERE (s.customer_id, s.start_date) IN (
...
select 
	round(avg(datediff(a.start_date, t.start_date)), 0) as avg_days_to_annual
from subscriptions as t
join subscriptions as a
...
SELECT 
    CONCAT(FLOOR(DATEDIFF(a.start_date, t.start_date)/30)*30 + 1, '-', 
           (FLOOR(DATEDIFF(a.start_date, t.start_date)/30)+1)*30, ' days') AS period,
    COUNT(*) AS customer_count
FROM subscriptions t
JOIN subscriptions a
  ON t.customer_id = a.customer_id
WHERE t.plan_id = 0     -- trial start
  AND a.plan_id = 3     -- pro annual
GROUP BY period
...
WHERE plan_id = 2   -- pro monthly
  AND next_plan = 1 -- next plan = basic monthly
